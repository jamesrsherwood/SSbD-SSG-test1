...
const SSBD = (function () {
  ...
  function _processEnergyMJ(s, st) {
    ...
    // 4. Stirring: Np × density [kg/m³] × N³ × d⁵ × 3600 × timeHr / η_stir / 1e6
    //    density ×1000 converts g/mL → kg/m³; η_stir = 0.90 (Piccinno 2016, Table 3, all scales)
    const eStir = (rp.Np * density * 1000 * Math.pow(rp.N, 3) * Math.pow(rp.d, 5)
                   * 3600 * timeHr) / (0.90 * 1e6);
  ...
  function step4(s, st) {
    ...
    // For distillation EOL, WK = CC from lost solvent + CC from distillation energy:
    //   wk = (1 − recovery%) × cc_c2g  +  distilEnergy_per_kg × CO2_intensity / 3600
    // This matches the spreadsheet (Database MX formula). Falls back to the simple
    // (1−recovery%) formula when distillation energy data is missing.
    ...
    if (eol === 'Recycled by distillation' && cc_c2g !== null) {
      ...
      if (distilMJ !== null && m_kg_d > 0) {
        const distilPerKg = distilMJ / m_kg_d;
        const recovery    = recov != null ? recov : 90.0;
        const co2Map_wk   = (typeof APP_OPTIONS !== 'undefined' && APP_OPTIONS.co2Intensity) || {};
        const region_wk   = (st && st.energyIntensity) || 'EU';
        const gco2_kwh_wk = co2Map_wk[region_wk] ?? 490;
        distil_cc = cc_c2g * (1 - recovery / 100) + distilPerKg * gco2_kwh_wk / 3600;
      }
    }
    ...
    // Distillation recovery: only the lost fraction (1−recovery) requires fresh production
    // water; the recovered fraction is reused. The 1.56 L/kg is the distillation process
    // water added on top regardless of recovery.
    const wu_eol = (eol === 'Recycled by distillation' && wu_c2g !== null)
                   ? wu_c2g * (1.0 - (recov != null ? recov : 90.0) / 100.0) + 1.56
                   : wu_c2g;
